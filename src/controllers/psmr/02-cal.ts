...
class BatchProcess {
    ...
    private functionId: string = 'BS01101';
    ...
    public async run(dateStr) {
        ...
        try {
            await writeDetailLog(this.functionId, this.moduleId, 'I', `Starting PSMR Cal02`, "BS00003INF", 'S', this.appId, this.userId);
            ...
            if (!rfqList) {
                writeDetailLog(this.functionId, this.moduleId, 'E', `Fail to retrieve the TB_M_PRD_RFQ`, "BS00001ERR", 'E', this.appId, this.userId);
                return;
            }

            if (!rfqDetailList) {
                writeDetailLog(this.functionId, this.moduleId, 'E', `Fail to retrieve the TB_M_PRD_RFQ_DT`, "BS00001ERR", 'E', this.appId, this.userId);
                return;
            }
            ...
            console.log('Cal02 process completed successfully');
        } catch (error) {
            console.error(error);
    ...
    private async retrieveRFQs(batchDate: Date): Promise<RFQ[]> {
        try {
            const sql = `SELECT * FROM TB_M_PRD_RFQ WHERE UPDATED_DATE = TO_DATE(:batchDate, 'YYYY-MM-DD')`;
            const result = await db.query(sql, {
                replacements: { batchDate : this.formattedDate(batchDate)  },
                type: QueryTypes.SELECT,
            });
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Fail to retrieve the TB_M_PRD_RFQ with : `+error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
    ...
    private async retrieveRFQDetails(batchDate: Date): Promise<RFQDetail[]> {
        try {
            const sql = `SELECT * FROM TB_M_PRD_RFQ_DT WHERE UPDATED_DATE = TO_DATE(:batchDate, 'YYYY-MM-DD')`;
            const result = await db.query(sql, {
                replacements: { batchDate : this.formattedDate(batchDate)  },
                type: QueryTypes.SELECT,
            });
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Fail to retrieve the TB_M_PRD_RFQ_DT with : `+error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
    ...
    private async insertPart(part: Part) {
        try {
            const sql = `INSERT INTO TB_R_OE_PO (
                TRIGGER_DATA_CAT, PART_NO, SUPPLIER_CODE, PROJECT_CODE, NAMC_CODE, 
                DOCK_CODE, COLOUR_CODE, INTERIM_PRICING_F, RFQ_CAT, RFQ_NO, 
                REV_NO, LV, STAGE, TRIGGER_TYPE, PART_TYPE, BUYER_CODE, 
                PART_NAME, TRIGGER_RECEIVED_DATE, SYSTEM_ASSIGNED_DUE_DATE, 
                FINAL_SRC_DUE_DATE, ECI_NO
            ) VALUES (
                :triggerDataCat, :partNo, :supplierCode, :projectCode, :namcCode, 
                :dockCode, :colourCode, :interimPricingF, :rfqCat, :rfqNo, 
                :revNo, :lv, :stage, :triggerType, :partType, :buyerCode, 
                :partName, TO_DATE(:triggerReceivedDate, 'YYYY-MM-DD'), 
                TO_DATE(:systemAssignedDueDate, 'YYYY-MM-DD'), 
                TO_DATE(:finalSrcDueDate, 'YYYY-MM-DD'), :eciNo
            );`;
            await db.query(sql, {
                replacements: {
                    triggerDataCat: part.TRIGGER_DATA_CAT,
                    ...
                    rfqNo: part.RFQ_NO,
                    revNo: part.REV_NO,
                    lv: part.PART_NO.substring(11, 12),
                    stage: part.STAGE,
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Error in inserting record into TB_R_OE_PO with `+error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
    ...
    private async updatePart(part: Part) {
        try {
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Error in updating record into TB_R_OE_PO with[1] `+error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
        ...
        try {
            const sql = `
                ...
                SET 
                    ...
                    REF_EFFECTIVE_TO = TO_DATE(:refEffectiveTo, 'YYYY-MM-DD'), 
            ...
            await db.query(sql, {
                replacements: {
                    ...
                    rfqEffectiveFrom: rfq.RFQ_EFFECTIVE_FROM || null, // Fallback to null
                    refEffectiveTo: rfq.REF_EFFECTIVE_TO || null, // Fallback to null
                    supplierCompletionDate: rfq.SUPPLIER_COMPLETION_DATE || null,
                    dueDate: rfq.DUE_DATE || null,
                    issuedDate: rfq.ISSUED_DATE || null,
                    receivedDate: rfq.RECEIVED_DATE || null,
                    buyerCheckDate: rfq.BUYER_CHECK_DATE || null,
                    buyerApprovalDate: rfq.BUYER_APPROVAL_DATE || null,
                    lastApprovalDate: rfq.LAST_APPROVAL_DATE || null,
                    massPrcApprovalCat: rfq.MASS_PRC_APPROVAL_CAT || null,
                    massPrcApprovalNo: rfq.MASS_PRC_APPROVAL_NO || null,
                    massPrcApprovalRevNo: rfq.MASS_PRC_APPROVAL_REV_NO || null,
                    rfqCat: rfq.RFQ_CAT,
                    rfqNo: rfq.RFQ_NO,
                    revNo: rfq.REV_NO,
                },
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Error in updating record into TB_R_OE_PO with[2] ` + error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
    ...
    private async isDuplicateRFQDetail(part): Promise<boolean> {
        try {
            const sql = `SELECT COUNT(*) AS count FROM TB_R_OE_PO_SUB WHERE TRIGGER_DATA_CAT = :triggerDataCat AND PART_NO = :partNo AND SUPPLIER_CODE = :supplierCode AND PROJECT_CODE = :projectCode AND NAMC_CODE = :namcCode AND DOCK_CODE = :dockCode AND COLOUR_CODE = :colourCode AND INTERIM_PRICING_F = :interimPricingF AND RFQ_CAT = :rfqCat AND RFQ_NO = :rfqNo AND REV_NO = :revNo`;
            const result = await db.query(sql, {
                replacements: {
                    triggerDataCat: part.TRIGGER_DATA_CAT,
                    partNo: part.PART_NO.substring(0,10),
    ...
    private async insertRFQDetail(rfqDetailList: RFQDetail[], part) {
        try {
            ...
            for (const rfqDetail of rfqDetailList) {
                ...
                if (rfqDetail.QUOTATION_DETAIL_CAT == 9){
            ...
            const sql_oe_po = `UPDATE TB_R_OE_PO 
            SET 
                MAT_DOM = :matDom, 
                PS_PART_DOM = :psPartDom, 
                PROCESS = :process, 
                SPECIAL_PKG = :specialPkg, 
                OH = :oh, MARK_UP = :markUp, 
            ...
            const sql_po_sub = `INSERT INTO TB_R_OE_PO_SUB (TRIGGER_DATA_CAT, PART_NO, SUPPLIER_CODE, PROJECT_CODE, NAMC_CODE, DOCK_CODE, COLOUR_CODE, INTERIM_PRICING_F, RFQ_CAT, RFQ_NO, REV_NO, MAT_IMP, PART_IMP, CURRENCY_CODE) VALUES (:triggerDataCat, :partNo, :supplierCode, :projectCode, :namcCode, :dockCode, :colourCode, :interimPricingF, :rfqCat, :rfqNo, :revNo, :matImp, :partImp, :currencyCode)`;
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Error in inserting record into TB_R_OE_PO_SUB with `+error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
    ...
    private async updateRFQDetail(rfqDetailList: RFQDetail[], part) {
        try {
            ...
            const sql = `UPDATE TB_R_OE_PO_SUB SET MAT_IMP = :matImp, PART_IMP = :partImp, CURRENCY_CODE = :currencyCode WHERE TRIGGER_DATA_CAT = :triggerDataCat AND PART_NO = :partNo AND SUPPLIER_CODE = :supplierCode AND PROJECT_CODE = :projectCode AND NAMC_CODE = :namcCode AND DOCK_CODE = :dockCode AND COLOUR_CODE = :colourCode AND INTERIM_PRICING_F = :interimPricingF AND RFQ_CAT = :rfqCat AND RFQ_NO = :rfqNo AND REV_NO = :revNo`;
            await db.query(sql, {
                replacements: {
                    matImp: MAT_IMP,
                    partImp: PART_IMP,
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Error in updating record into TB_R_OE_PO_SUB with `+error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
    ...
    private async updatePO(po: PO) {
        try {
            const sql = `UPDATE TB_R_OE_PO SET SENT_PIECE_PO_DATE = :sentPiecePoDate, PO_NO = :poNo, PO_REVISION_NO = :poRevisionNo WHERE PO_NO = :poNo AND PO_REVISION_NO = :poRevisionNo`;
            await db.query(sql, {
                replacements: {
            ...
            writeDetailLog(this.functionId, this.moduleId, 'E', `Error in updating record into TB_R_OE_PO with[3] `+error.message, "BS00001ERR", 'E', this.appId, this.userId);
            // throw error;
        }
    }
    ...
    private async sendErrorEmail() {
        if (this.logCollection.length > 0) {
            try {
                await sendToPIC(this.logCollection, 'PSMR00001', "seksit39@outlook.com", undefined, undefined, db);
            } catch (error) {
